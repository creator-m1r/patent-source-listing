...
final class ListingViewModel: ObservableObject {
    ...
    func chooseFolder() {
        let panel = NSOpenPanel(); panel.title = "Выберите корневую папку проекта"; panel.canChooseDirectories = true; panel.canChooseFiles = false; panel.allowsMultipleSelection = false
    ...
    func scan() {
        ...
        Task.detached(priority: .userInitiated) {
            do {
                let result = try scanner.scan(root: folder)
    ...
    func exportRTF() {
        ...
        let panel = NSSavePanel(); panel.title = "Сохранить листинг"; panel.nameFieldStringValue = (metadata.projectName.isEmpty ? "Program" : metadata.projectName) + "_Source_Listing.rtf"; panel.allowedContentTypes = [.rtf]; panel.canCreateDirectories = true
        guard panel.runModal() == .OK, let url = panel.url else { return }
        do { try exporter.export(metadata: metadata, report: report, to: url); status = "RTF сохранен: \(url.lastPathComponent)" } catch { errorMessage = "Ошибка экспорта: \(error.localizedDescription)" }
    }
}
