    ...
    var body: some View {
            ...
            Form {
                ...
                Section("Профили") {
                    ...
                    ForEach(model.profiles) { profile in
                        HStack { Button(profile.name) { model.loadProfile(profile) }.buttonStyle(.plain); Spacer(); Button { model.deleteProfile(profile) } label: { Image(systemName: "trash") }.buttonStyle(.borderless) }
                    }
                ...
                Section("Состав листинга") {
                    ...
                    Text("Исходные файлы на диске не изменяются. Комментарии удаляются только из копии текста внутри RTF-листинга.").font(.caption).foregroundStyle(.secondary)
                ...
                Section("Проверка") { Button { showValidation = true } label: { Label("Проверить листинг", systemImage: "checkmark.shield") } }
                Section("Экспорт") {
        ...
        .sheet(isPresented: $showProfileSheet) {
            VStack(alignment: .leading, spacing: 16) {
                Text("Новый профиль").font(.title2.bold()); TextField("Название профиля", text: $newProfileName)
                HStack { Spacer(); Button("Отмена") { showProfileSheet = false }; Button("Сохранить") { model.saveProfile(named: newProfileName); newProfileName = ""; showProfileSheet = false }.keyboardShortcut(.defaultAction).disabled(newProfileName.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty) }
...
private struct DetailView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            HStack { VStack(alignment: .leading) { Text("Листинг исходного текста").font(.largeTitle.bold()); Text("Подготовка материалов проекта для передачи патентной организации").foregroundStyle(.secondary) }; Spacer(); Toggle("Дерево", isOn: $showTree).toggleStyle(.switch) }
            HStack { Metric(title: "Файлов", value: "\(model.report.files.count)"); Metric(title: "Строк", value: "\(model.report.totalLines)"); Metric(title: "Размер", value: ByteCountFormatter.string(fromByteCount: model.report.totalBytes, countStyle: .file)); Metric(title: "Игнорировано", value: "\(model.report.ignoredCount)") }
            ...
            if showTree && !model.report.files.isEmpty { GroupBox("Древовидная структура") { ScrollView { Text(TreeBuilder.build(files: model.report.files)).font(.system(.caption, design: .monospaced)).textSelection(.enabled).frame(maxWidth: .infinity, alignment: .leading).padding(8) } }.frame(maxHeight: 260) }
            ...
            if model.configuration.removeComments { Label("При экспорте комментарии будут удалены из текста, но оригинальные файлы останутся неизменными.", systemImage: "eraser").font(.caption).foregroundStyle(.secondary) }
            Text("Файлы в логическом порядке").font(.headline)
            List(model.report.files) { file in VStack(alignment: .leading, spacing: 3) { Text(file.relativePath).font(.system(.body, design: .monospaced)); Text("\(file.language) · \(file.lineCount) строк · \(file.size) байт").font(.caption).foregroundStyle(.secondary) }.padding(.vertical, 3) }
        }.padding(24)
    }
}
...
private struct Metric: View { let title: String; let value: String; var body: some View { VStack(alignment: .leading) { Text(value).font(.title2.bold()); Text(title).font(.caption).foregroundStyle(.secondary) }.frame(maxWidth: .infinity, alignment: .leading) } }
